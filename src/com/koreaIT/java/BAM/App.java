...
public class App {
	...
	private List<Article> articles;   // static 제거, 접근제어자를 private로 수정 
	
	App() {	                          
		articles = new ArrayList<>();   // articles 초기화.  선언과 동시에 초기화를 안한다
	}
	
	public void run() {        //main() -> run() 수정, static 제거, 매개변수 제거
...
//				Article article = new Article(id, regDate, title, body, 0);  4번째 조회수 매개변수를 0으로 지정해도 됨
				...
				System.out.println("번호	|	제목	|	내용	|	조회	|	날짜");
				for (int i = articles.size() - 1; i >= 0; i--) {
					Article article = articles.get(i);
				System.out.printf("%d	|	%s	|	%s	|	%d	|	%s\n", article.id, article.title, article.body, article.viewCnt, article.regDate );
				}
	...
	private void makeTestData() {  // 접근제어자를 private로 수정, static 제거
								   //  private 인데 문제가 없는 이유는?
